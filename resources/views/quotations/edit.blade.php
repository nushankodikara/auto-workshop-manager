...
@section('title', 'Revise Service Quotation')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-3 border-b border-slate-200 dark:border-slate-800 pb-4">
        <a href="{{ route('quotations.show', $quotation->id) }}" class="text-sm font-semibold text-primary hover:underline flex items-center gap-1">
            <i data-lucide="arrow-left" class="w-3.5 h-3.5"></i>
            <span>Back to Details</span>
        </a>
        <span class="text-slate-400">|</span>
        <span class="text-slate-650 dark:text-slate-350 font-semibold text-sm">Revise Quotation #{{ $quotation->quotation_number }}</span>
    </div>

    <form action="{{ route('quotations.update', $quotation->id) }}" method="POST" id="quotation-form" class="space-y-6">
        @csrf
        @method('PUT')

        <!-- Revision Reason Log (Mandatory) -->
        <div class="app-card rounded-2xl p-6 space-y-4 shadow-xs border border-amber-500/20 bg-amber-500/5">
            <h3 class="text-sm font-bold uppercase tracking-wider text-amber-600 dark:text-amber-400 pb-2 flex items-center gap-2 border-b border-amber-500/10">
                <i data-lucide="history" class="w-4 h-4 text-amber-500 animate-pulse"></i>
                <span>Revision Changelog / Edit Reason</span>
            </h3>
            <div>
                <label for="revision_reason" class="block text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2">Reason for Revision <span class="text-red-500">*</span></label>
                <textarea name="revision_reason" id="revision_reason" rows="2" required placeholder="e.g., Customer requested changing the brake pad brands to OEM instead of aftermarket..."
                          class="w-full px-4 py-2.5 app-input rounded-lg text-slate-900 dark:text-slate-200 focus:outline-none focus:border-amber-400 text-sm border-amber-400/30"></textarea>
                <p class="text-[11px] text-slate-400 mt-1 leading-normal">Specify the changes requested. This explanation is saved permanently in the quotation history.</p>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="customer_address" id="customer_address" placeholder="e.g. 123 Galle Road, Colombo"
                           value="{{ old('customer_address', $quotation->customer_address) }}"
                           class="w-full px-4 py-2.5 app-input rounded-lg text-slate-900 dark:text-slate-200 focus:outline-none focus:border-primary text-sm">
                </div>
            </div>
        </div>
            ...
            <div class="app-card rounded-2xl p-6 space-y-4 shadow-xs">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" step="0.01" min="0" max="100" name="discount_percent" id="discount_percent" value="{{ old('discount_percent', $quotation->discount_percent) }}" oninput="calculateTotals()"
                               class="w-full px-4 py-2.5 app-input rounded-lg text-slate-900 dark:text-slate-200 focus:outline-none focus:border-primary text-sm font-mono font-semibold">
                    </div>
                    <div>
                        ...
                        <input type="number" step="0.01" min="0" name="tax" id="tax" value="{{ old('tax', $quotation->tax) }}" oninput="calculateTotals()"
                               class="w-full px-4 py-2.5 app-input rounded-lg text-slate-900 dark:text-slate-200 focus:outline-none focus:border-primary text-sm font-mono font-semibold">
                    </div>
                </div>
            </div>
        ...
        <div class="flex justify-end gap-3">
            <a href="{{ route('quotations.show', $quotation->id) }}" class="px-5 py-2.5 bg-slate-200 hover:bg-slate-300 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 rounded-lg text-xs font-bold transition flex items-center justify-center">
                Cancel
            </a>
...
<script>
        ...
        row.innerHTML = `
            <td class="py-3 pr-4">
                <input type="text" name="items[${rowIndex}][description]" required value="${description}" placeholder="e.g. Brake Caliper Lubricant, Oil Filter Exchange"
                       class="w-full px-3 py-1.5 app-input rounded-lg text-slate-900 dark:text-slate-200 focus:outline-none focus:border-primary text-xs font-semibold">
            </td>
            <td class="py-3 px-2">
            ...
            <td class="py-3 pl-4 text-right">
                <button type="button" onclick="removeRow(${rowIndex})" class="text-red-500 hover:text-red-400 p-1.5 cursor-pointer">
                    ✕
                </button>
            </td>
        `;
    ...
    // Initialize existing quotation items
    window.addEventListener('DOMContentLoaded', () => {
        @foreach($quotation->items as $item)
            addCustomRow('{{ $item->type }}', '{!! addslashes($item->description) !!}', {{ floatval($item->quantity) }}, {{ floatval($item->unit_price) }});
        @endforeach
