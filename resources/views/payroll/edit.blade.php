...
<div class="space-y-6 max-w-4xl mx-auto">
    ...
    <div class="flex items-center gap-3 border-b border-slate-200 dark:border-slate-800 pb-4">
        <a href="{{ route('payroll.show', $payrollSlip->id) }}" class="text-sm font-semibold text-primary hover:underline">
            &larr; Back to Slip
        </a>
        <span class="text-slate-400">|</span>
        <span class="text-slate-655 dark:text-slate-350 font-semibold text-sm capitalize">Edit Payslip for {{ $user->name }}</span>
    </div>

    <!-- Edit Form -->
    <form action="{{ route('payroll.update', $payrollSlip->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        
        <input type="hidden" name="total_salary" value="{{ $payrollSlip->total_salary }}">
        ...
        <div class="app-card rounded-2xl p-6 grid grid-cols-1 md:grid-cols-4 gap-6 items-center shadow-xs">
            ...
            <div>
                ...
                <span class="font-semibold text-slate-800 dark:text-slate-200 text-sm block">
                    {{ date('F Y', mktime(0, 0, 0, $payrollSlip->month, 1, $payrollSlip->year)) }}
                </span>
            </div>
            <div>
                ...
                <span class="font-mono font-bold text-slate-850 dark:text-slate-200 text-sm block">
                    {{ config('app.currency', 'Rs.') }}{{ number_format($payrollSlip->basic_salary, 2) }}
                </span>
                <input type="hidden" id="contract_salary" value="{{ $payrollSlip->basic_salary }}">
            </div>
            <div>
                <span class="text-xs text-slate-500 uppercase tracking-wider block font-semibold mb-1">Contract Total Salary</span>
                <span class="font-mono font-bold text-slate-850 dark:text-slate-200 text-sm block">
                    {{ config('app.currency', 'Rs.') }}{{ number_format($payrollSlip->total_salary, 2) }}
                </span>
            </div>
        </div>
        ...
        <div class="app-card rounded-2xl p-6 space-y-4 shadow-xs">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div>
                    ...
                    <input type="number" name="required_days" id="required_days" value="{{ $payrollSlip->required_days }}" min="1" max="31" oninput="recalculate()"
                ...
                <div>
                    ...
                    <input type="number" step="any" name="attended_days" id="attended_days" value="{{ $payrollSlip->attended_days }}" min="0" max="31" oninput="recalculate()"
                           class="w-full px-4 py-2 app-input rounded-lg text-slate-900 dark:text-slate-200 font-mono text-sm focus:outline-none focus:border-primary">
                </div>
                <div>
        ...
        <div class="app-card rounded-2xl p-6 space-y-4 shadow-xs">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="overtime_rate" id="overtime_rate" value="{{ $payrollSlip->overtime_rate }}" oninput="recalculate()"
                           class="w-full px-4 py-2 app-input rounded-lg text-slate-900 dark:text-slate-200 font-mono text-sm focus:outline-none focus:border-primary">
                </div>
                <div>
                    ...
                    <input type="number" step="0.01" name="overtime_amount" id="overtime_amount" value="{{ $payrollSlip->overtime_amount }}" oninput="recalculateTotal()"
                           class="w-full px-4 py-2 app-input rounded-lg text-slate-900 dark:text-slate-200 font-mono text-sm focus:outline-none focus:border-primary">
                </div>
            </div>
            <p class="text-[10px] text-slate-500 leading-normal">
        ...
        <div class="app-card rounded-2xl p-6 space-y-4 shadow-xs">
                ...
                @foreach($payrollSlip->items->where('type', 'addition') as $item)
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-center addition-row">
                        <div class="md:col-span-3">
                            <input type="text" name="item_name[]" value="{{ $item->category_name }}" placeholder="Description" required
                        ...
                        <div class="flex gap-2">
                            <input type="number" step="0.01" name="item_amount[]" placeholder="0.00" value="{{ $item->amount }}" oninput="recalculateTotal()"
                                   class="flex-1 px-4 py-2 app-input rounded-lg text-slate-900 dark:text-slate-200 font-mono text-xs focus:outline-none focus:border-primary addition-amount">
                            <button type="button" onclick="this.closest('.addition-row').remove(); recalculateTotal();" class="text-red-500 hover:text-red-400 font-bold px-2 text-sm cursor-pointer">✕</button>
                        </div>
                    </div>
                @endforeach
            </div>
            ...
            <button type="button" onclick="addRow('addition', 'additions-container')"
                    class="py-1.5 px-3 bg-slate-200 hover:bg-slate-300 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-semibold rounded-lg text-xs transition border border-transparent hover:border-slate-300 dark:hover:border-slate-700/50 flex items-center gap-1 cursor-pointer">
        ...
        <div class="app-card rounded-2xl p-6 space-y-4 shadow-xs">
            <h3 class="text-sm font-bold uppercase tracking-wider text-slate-550 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800 pb-3 flex items-center gap-2">
                <i data-lucide="minus-circle" class="w-4 h-4 text-red-655 dark:text-red-400"></i>
            ...
            <div id="deductions-container" class="space-y-3">
                @foreach($payrollSlip->items->where('type', 'deduction') as $item)
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-center deduction-row">
                        ...
                        <div class="flex gap-2">
                            <input type="number" step="0.01" name="item_amount[]" placeholder="0.00" value="{{ $item->amount }}" oninput="recalculateTotal()"
                                   class="flex-1 px-4 py-2 app-input rounded-lg text-slate-900 dark:text-slate-200 font-mono text-xs focus:outline-none focus:border-primary deduction-amount">
                            <button type="button" onclick="this.closest('.deduction-row').remove(); recalculateTotal();" class="text-red-500 hover:text-red-400 font-bold px-2 text-sm cursor-pointer">✕</button>
                        </div>
                    </div>
                @endforeach
            </div>
        ...
        <div class="app-card rounded-2xl p-6 space-y-4 shadow-xs">
            ...
            <div id="benefits-container" class="space-y-3">
                @foreach($payrollSlip->items->where('type', 'benefit') as $item)
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-center benefit-row">
                        ...
                        <div class="flex gap-2">
                            <input type="number" step="0.01" name="item_amount[]" placeholder="0.00" value="{{ $item->amount }}" oninput="recalculateTotal()"
                                   class="flex-1 px-4 py-2 app-input rounded-lg text-slate-900 dark:text-slate-200 font-mono text-xs focus:outline-none focus:border-primary benefit-amount">
                            <button type="button" onclick="this.closest('.benefit-row').remove(); recalculateTotal();" class="text-red-500 hover:text-red-400 font-bold px-2 text-sm cursor-pointer">✕</button>
                        </div>
                    </div>
                @endforeach
            </div>
        ...
        <div class="flex justify-end gap-3 pt-4 border-t border-slate-200 dark:border-slate-850">
            <a href="{{ route('payroll.show', $payrollSlip->id) }}"
            ...
            <button type="submit"
                    class="py-2.5 px-5 bg-primary hover:bg-primary-hover text-white font-semibold rounded-lg text-xs transition shadow-sm cursor-pointer">
                Save & Update Salary Slip
            </button>
        </div>
