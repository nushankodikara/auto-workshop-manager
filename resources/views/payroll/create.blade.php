...
<div class="space-y-6 max-w-3xl mx-auto">
    ...
    <form action="{{ route('payroll.store') }}" method="POST" class="space-y-8">
        ...
        <!-- 1. Employee Context -->
        <div class="app-card rounded-2xl p-6 grid grid-cols-1 md:grid-cols-3 gap-6 items-center shadow-xs">
            <div>
            ...
            <div>
                <span class="text-xs text-slate-500 uppercase tracking-wider block font-semibold mb-1">Contract Base Salary</span>
                <span class="font-mono font-bold text-slate-850 dark:text-slate-200 text-sm block">
                    {{ config('app.currency', '$') }}{{ number_format($user->basic_salary, 2) }}
                </span>
        ...
        <div class="app-card rounded-2xl p-6 space-y-4 shadow-xs">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                ...
                <div>
                    <label for="prorated_salary" class="block text-xs text-slate-500 mb-1 font-semibold">Prorated Base Salary ({{ config('app.currency', '$') }})</label>
            ...
            <p class="text-[10px] text-slate-500 leading-normal">
                Formula: (Attended Days / Required Days) * Contract Base Salary. Calculated automatically, but can be overridden manually if needed.
            </p>
        </div>
        ...
        <div class="app-card rounded-2xl p-6 space-y-4 shadow-xs">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div>
                    ...
                    <input type="number" step="0.5" name="overtime_hours" id="overtime_hours" value="{{ $overtimeHours }}" oninput="recalculate()"
                           class="w-full px-4 py-2 app-input rounded-lg text-slate-900 dark:text-slate-200 font-mono text-sm focus:outline-none focus:border-primary">
                </div>
                <div>
                    <label for="overtime_rate" class="block text-xs text-slate-500 mb-1 font-semibold">Hourly OT Rate ({{ config('app.currency', '$') }})</label>
                    <input type="number" step="0.01" name="overtime_rate" id="overtime_rate" value="{{ $overtimeRate }}" oninput="recalculate()"
                           class="w-full px-4 py-2 app-input rounded-lg text-slate-900 dark:text-slate-200 font-mono text-sm focus:outline-none focus:border-primary">
                </div>
                <div>
                    <label for="overtime_amount" class="block text-xs text-slate-500 mb-1 font-semibold">OT Payout Amount ({{ config('app.currency', '$') }})</label>
