...
- **Towing & Transportation Tracking**: Added `transportation_fee` and `transportation_type` fields (provided vs hire) on Job Cards.
  * Billing: Credits Transportation Revenue (`4200`) instead of service revenue.
  * Payment: Debits Transportation Account (`1030`) for towing portion, net cash remainder debits Cashbook (`1000`). If paid via advanced payments, net cash settles automatically by debiting `1030` and crediting Cashbook (`1000`) for the towing fee portion.
  * Hire Type: Creates an additional transaction debiting Transportation Hire Expense (`5500`) and crediting Transportation Account (`1030`) for third-party driver payout.
  * Retroactive Repair Services Cleanup: Includes settings maintenance utility to scan legacy tasks matching keywords (e.g. `transport`, `towing`) and convert them to formal Job Card fields with bookkeeping re-allocations.
